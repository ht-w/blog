          ...
          <article class="test d-flex flex-column flex-sm-row mb-10"  >
            <v-row dense>
              <v-col cols="12" class="pt-5">
                <h2 class="pl-3">
                  <a  class="articleTitle"   @click="$router.push(`/article/post/${article.id}`);">{{article.title}}</a>
                </h2>
                <time class="datetime pl-3">{{article.createTime| dateformat('DD MMM YYYY')}}</time>
                <a class="category">#{{article.categoryName}}</a>
...
props: ['cid'],
  ...
  methods:{
    async getPostList(){
      const {data:res} = await this.$http.get(`api/post/${this.pageNum}/${this.pageSize}/${this.cid}`)
